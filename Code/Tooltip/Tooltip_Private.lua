...
do
    ...
    function PreviewModelMixin:Init(parent)
        ...
        if addon.IS_LEGACY_ASCENSION then
            -- Decor model scenes do not exist in 3.3.5. Keep a hidden layout
            -- stand-in while retaining ordinary DressUpModel item previews.
            ModelScene = CreateFrame("Frame", nil, parent);
            ModelScene.TransitionToModelSceneID = API.Nop;
            ModelScene.GetActorByTag = API.Nop;
        else
            ModelScene = CreateFrame("ModelScene", nil, parent, "PanningModelSceneMixinTemplate");
        end
...
do
    local function HidePreviewModel(self)
        self.usePreviewModel = false;
        self.PreviewFrame:Hide();
    end

    if addon.IS_LEGACY_ASCENSION then
        -- Wrath has a fully usable DressUpModel for equipment, but it has none
        -- of Retail's pet, mount, transmog-source, or housing model APIs.  Keep
        -- the useful equipment preview instead of disabling the whole feature
        -- when C_PetJournal is unavailable.
        function SharedTooltip:DisplayModel()
            local itemID = self:GetItemID();
            if not (itemID and IsDressableItemByID and IsDressableItemByID(itemID)) then
                HidePreviewModel(self);
                return
            end
            ...
            PreviewModel:TryOn(self:GetHyperlink() or ("item:"..itemID));
            PreviewModel:SetModelSize(MODEL_WIDTH, MODEL_HEIGHT);
            PreviewModel:SetOffsetY(0.33);
            PreviewModel:SetUseTurntable(false);
...
function SharedTooltip:ProcessInfo(info)
    ...
    local getter = C_TooltipInfo and C_TooltipInfo[info.getterName];
    ...
    if info.getterArgs then
        tooltipData = getter(unpack(info.getterArgs));
    else
        tooltipData = getter();
    end

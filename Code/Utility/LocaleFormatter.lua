...
local HOTKEY_PATH = "Interface/AddOns/DialogueUI-Ascension/Art/Keys/";
local BUTTON_FORMAT = {
    TAB = "|TInterface/AddOns/DialogueUI-Ascension/Art/Keys/PC-TAB.tga:16:29:0:-4:64:32:0:58:0:32:|t";
}
...
do
    ...
    local function ReplaceStringWithKey_Recursive(text, offsetY)
        ...
        if type and type == "KEY" and arg1 and arg2 then
            ...
            if not texture then
                texture = ("|T%s%s-%s.tga:%s:%s:0:%s|t"):format(HOTKEY_PATH, device, button, iconSize, iconSize, offsetY);
            end

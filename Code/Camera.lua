...
local HAS_CAMERA_ZOOM = type(GetCameraZoom) == "function" and type(CameraZoomIn) == "function" and type(CameraZoomOut) == "function";
local ConsoleExec = ConsoleExec;
-- SetUIVisibility changes protected UI state on Wrath.  Calling it from
-- insecure addon code can taint action bars and unrelated protected actions.
-- The legacy port therefore never hides or restores UIParent.
local SetGameUIShown = not addon.IS_LEGACY_ASCENSION and SetUIVisibility;
...
local function GetExistingCVar(cvar)
...
local function BackupAndSetCVar(cvar, value)
    if CVar_Backup[cvar] == nil then
        CVar_Backup[cvar] = GetExistingCVar(cvar);
    end
    if CVar_Backup[cvar] == nil then
        return false
    end
...
function CameraUtil:ChangeCVars()
    ...
    if CHANGE_FOV and not IsDynamicFlying() then
        FOV_DEFAULT = tonumber(GetExistingCVar("cameraFov")) or 90;
...
local function ShowUIParent(state)
    if not SetGameUIShown then return end;
...
function FadeHelper:SnapToFadeResult(inCombat)
    ...
    if not SetGameUIShown then
        -- UIParent is protected on legacy Wrath. Even a redundant SetAlpha(1)
        -- can taint later action-bar and logout calls.
        self.fadeDelta = nil;
        self.owner = nil;
        return
    end
...
do
    local function Settings_CameraMovement(dbValue)
        -- Original Wrath exposes relative zoom commands but no readable camera
        -- distance. Disabling motion is the only way to guarantee exact camera
        -- restoration after a conversation.
        CameraUtil:SetDefaultCameraMode(HAS_CAMERA_ZOOM and dbValue or 0)
    end
    ...
    local function Settings_HideUI(dbValue, userInput)
        HIDE_UI = not addon.IS_LEGACY_ASCENSION and dbValue == true;
...
if not addon.IS_LEGACY_ASCENSION and UIModeUtil and UIModeUtil.RegisterMode and UIModeUtil.SetModeActive then

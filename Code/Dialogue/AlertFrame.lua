...
function AlertFrame:OnShow()
    ...
    self:RegisterEvent(addon.IS_LEGACY_ASCENSION and "LFG_PROPOSAL_SUCCEEDED" or "LFG_PROPOSAL_DONE");
...
function AlertFrame:OnHide()
    ...
    self:UnregisterEvent(addon.IS_LEGACY_ASCENSION and "LFG_PROPOSAL_SUCCEEDED" or "LFG_PROPOSAL_DONE");
...
function AlertFrame:OnEvent(event, ...)
    ...
    if event == "UI_INFO_MESSAGE" then
        if addon.IS_LEGACY_ASCENSION then
            self:QueueMessage((...), COLOR_YELLOW);
        else
            self:TryDisplayMessage(...);
        end
    elseif event == "UI_ERROR_MESSAGE" then
        if addon.IS_LEGACY_ASCENSION then
            self:QueueMessage((...), COLOR_RED);
        else
            self:TryDisplayMessage(...);
        end
    elseif event == "QUEST_ACCEPTED" then
        ...
        elseif classicQuestID and classicQuestID ~= 0 then
    ...
    elseif event == "LFG_PROPOSAL_SHOW" then
        self:OnLFGProposal();
    elseif event == "LFG_PROPOSAL_DONE" or event == "LFG_PROPOSAL_SUCCEEDED" or event == "LFG_PROPOSAL_FAILED" then
        self:HideLFGDialog();
    end
end
...
function AlertFrame:ShowLFGDialog()
    if not LFGAlert.initialized then
        ...
        LFGAlert.Icon:SetTexture("Interface/AddOns/DialogueUI-Ascension/Art/Theme_Shared/LFGEye.tga");
        ...
        LFGAlert.Background:SetTexture("Interface/AddOns/DialogueUI-Ascension/Art/Theme_Shared/TopAlertBackground.tga");
...
do
    local function Settings_HideUI(dbValue)
        ALERT_FRAME_ENABLED = not addon.IS_LEGACY_ASCENSION and (dbValue == true);
    end

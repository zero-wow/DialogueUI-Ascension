...
do
    local function CreateHoldAnimation(group, duration, order)
        local animation = group:CreateAnimation("Animation");
        animation:SetDuration(duration);
        animation:SetOrder(order);
    end

    local function CreateContentAnimation(region)
        local group = region:CreateAnimationGroup();

        local offsetIn = group:CreateAnimation("Translation");
        offsetIn:SetOffset(-12, 0);
        offsetIn:SetDuration(0);
        offsetIn:SetOrder(1);

        local fadeInStart = group:CreateAnimation("Alpha");
        fadeInStart:SetChange(-1);
        fadeInStart:SetDuration(0);
        fadeInStart:SetOrder(1);

        CreateHoldAnimation(group, 0.5, 1);
        CreateHoldAnimation(group, 1.5, 2);

        local offsetOut = group:CreateAnimation("Translation");
        offsetOut:SetOffset(12, 0);
        offsetOut:SetDuration(0.25);
        offsetOut:SetOrder(3);
    ...
    local function CreateAlertAnimation(region)
        local group = region:CreateAnimationGroup();
        CreateHoldAnimation(group, 0.5, 1);
        CreateHoldAnimation(group, 1.5, 2);
    ...
    function QuestFlyoutFrameMixin:Init()
        ...
        self.ContentAnimIn = CreateContentAnimation(ContentFrame);
        self.AlertAnimOut = CreateAlertAnimation(Alert);

        self.AnimIn:SetScript("OnPlay", function()
            self.ContentAnimIn:Stop();
            self.AlertAnimOut:Stop();
            ContentFrame:SetAlpha(1);
            Alert:SetAlpha(1);
            self.ContentAnimIn:Play();
            self.AlertAnimOut:Play();
        end);
        self.AnimIn:SetScript("OnStop", function()
            self.ContentAnimIn:Stop();
            self.AlertAnimOut:Stop();
            ContentFrame:SetAlpha(1);
            Alert:SetAlpha(1);
        end);
        self.AlertAnimOut:SetScript("OnFinished", function()
            Alert:SetAlpha(0);
        end);
    end
    ...
    function QuestFlyoutFrameMixin:OnEvent(event, ...)
        ...
        elseif event == "UI_ERROR_MESSAGE" then
            ...
            if addon.IS_LEGACY_ASCENSION or QUEST_ERROR_TYPES[errorType] then
                self:SetWatchedQuest(nil);

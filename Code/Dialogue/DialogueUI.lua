...
local GetAvailableTitle = (addon.IS_LEGACY_ASCENSION and addon.Legacy.GetAvailableTitle) or GetAvailableTitle;
...
local GetActiveTitle = (addon.IS_LEGACY_ASCENSION and addon.Legacy.GetActiveTitle) or GetActiveTitle;
local SelectActiveQuest = (addon.IS_LEGACY_ASCENSION and addon.Legacy.SelectActiveQuest) or SelectActiveQuest;
...
function DUIDialogBaseMixin:OnLoad()
    ...
    local function CreateTextBackground()
        ...
        texture:SetTexture(ThemeUtil:GetTextureFile("SubHeaderBackground.tga"));
    ...
    if not addon.IS_LEGACY_ASCENSION then
        ...
        -- 3.3.5 has no LOADING_SCREEN_DISABLED.  Initialization translates
        -- PLAYER_ENTERING_WORLD into this callback, so release deferred
        -- gossip/quest events here instead of leaving the dialog permanently
        -- in its loading state.
        CallbackRegistry:RegisterLoadingCompleteCallback(function()
...
function DUIDialogBaseMixin:LoadTheme()
    ...
    if self.textBackgroundPool then
        local bgFile = ThemeUtil:GetTextureFile("SubHeaderBackground.tga");
...
function DUIDialogBaseMixin:IsGossipHandledExternally()
    ...
    if addon.IS_LEGACY_ASCENSION and addon.Legacy and addon.Legacy.IsExcludedInteraction then
        return addon.Legacy.IsExcludedInteraction()
    end
    return false
end
...
function DUIDialogBaseMixin:UpdateRewards()
    if self.questLayout and self.handler then
        ...
        if not self.rewardUpdator then
            self.rewardUpdator = CreateFrame("Frame", nil, self);
...
function DUIDialogBaseMixin:OnEvent(event, ...)
    ...
    elseif event == "GOSSIP_CONFIRM" then
        ...
        if addon.IS_LEGACY_ASCENSION then
            -- Stock Wrath creates its duplicate popup from UIParent.  Hide it
            -- after event dispatch instead of tainting UIParent by changing
            -- its registered events.
            After(0, API.CloseGossipStaticPopups);
        end
...
function DUIDialogBaseMixin:ScrollDownOrAcceptQuest(fromMouseClick)
    ...
    -- Preserve Wrath's warning before a quest flags the player for PvP.  The
    -- stock popup's accept callback performs AcceptQuest; if a custom client
    -- removed that popup definition, fall through to the normal path.
    if not self.acknowledgeAutoAcceptQuest
        and QuestFlagsPVP and QuestFlagsPVP()
        and StaticPopup_Show and StaticPopupDialogs
        and StaticPopupDialogs["CONFIRM_ACCEPT_PVP_QUEST"] then
...
do  --Vignette
    ...
    SharedVignette.Texture:SetTexture("Interface/AddOns/DialogueUI-Ascension/Art/Theme_Shared/ScreenVignette.tga");

...
local GetRewardTalents = GetRewardTalents or API.AlwaysZero;
...
local function BuildRewardList(questComplete)
    ...
    talentPoints = GetRewardTalents() or 0;
	...
	if ( totalRewards == 0 and money == 0 and xp == 0 and not playerTitle and talentPoints == 0 and #spellRewards == 0 and artifactXP == 0 and honor == 0 and not majorFactionRepRewards ) then
		return nil;
	end


    if ( artifactXP > 0 ) and C_ArtifactUI and C_ArtifactUI.GetArtifactXPRewardTargetInfo then
	...
	local anyGenericAward = playerTitle or talentPoints > 0 or numQuestRewards > 0 or numQuestCurrencies > 0 or money > 0 or xp > 0 or honor > 0 or majorFactionRepRewards or hasChanceForQuestSessionBonusReward;
	...
	if anyGenericAward then
		...
		-- Wrath quests can directly award talent points; Retail removed this
		-- reward type, so upstream DialogueUI has no representation for it.
		if talentPoints > 0 then
			tinsert(rewardList, {"SetRewardTalent", talentPoints, order = 240});
		end
		...
		-- warmode bonus
		local order_pvp = 500;
		if hasWarModeBonus and C_PvP and C_PvP.IsWarModeDesired and C_PvP.GetWarModeRewardBonus and C_PvP.IsWarModeDesired() then

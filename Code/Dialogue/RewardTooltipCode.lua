...
local function ShowLegacyRewardTooltip(tooltip, itemButton, fallbackTitle)
    ...
    title = title or REWARD or "Reward";

    local amount;
    if itemButton.Count and itemButton.Count.GetText then
        amount = itemButton.Count:GetText();
    end

    if tooltip.SetTitle then
        tooltip:SetTitle(title, 1, 0.82, 0);
        if amount and amount ~= "" then
            tooltip:AddLeftLine((REWARD or "Reward")..": "..amount, 1, 1, 1);
        end
    else
        tooltip:SetText(title, 1, 0.82, 0);
        if amount and amount ~= "" then
            tooltip:AddLine((REWARD or "Reward")..": "..amount, 1, 1, 1);
        end
    end
    tooltip:Show();
end
...
local function AppendQuestRewardContextDescription(self, tooltip)
    ...
    if rewardContextLine then
        ...
        elseif GameTooltip_AddColoredLine then
            GameTooltip_AddColoredLine(tooltip, rewardContextLine, QUEST_REWARD_CONTEXT_FONT_COLOR);
        elseif tooltip.AddLine then
            tooltip:AddLine(rewardContextLine, QUEST_REWARD_CONTEXT_FONT_COLOR.r, QUEST_REWARD_CONTEXT_FONT_COLOR.g, QUEST_REWARD_CONTEXT_FONT_COLOR.b);
        end
...
local function CustomTooltip_OnEnter(self)
    ...
    elseif self.objectType == "currency" then
        local shown = tooltip.SetQuestCurrency and tooltip:SetQuestCurrency(self.type, self.index);
        if addon.IS_LEGACY_ASCENSION and not shown then
            ShowLegacyRewardTooltip(tooltip, self, _G.CURRENCY or "Currency");
        end
    ...
    elseif self.objectType == "skill" then
        ...
        local info = self.skillLineID and C_TradeSkillUI and C_TradeSkillUI.GetProfessionInfoBySkillLineID and C_TradeSkillUI.GetProfessionInfoBySkillLineID(self.skillLineID);
        if info then
    ...
    elseif self.objectType == "honor" then
        local shown = tooltip.SetCurrencyByID and tooltip:SetCurrencyByID(self.currencyID);
        if addon.IS_LEGACY_ASCENSION and not shown then
            ShowLegacyRewardTooltip(tooltip, self, L["Honor Points"]);
        end
    else
        tooltip:Hide();
    end
...
local function GameTooltip_OnEnter(self)
    ...
    elseif self.objectType == "reputation" then
        ...
        local title = QUEST_REPUTATION_REWARD_TITLE:format(self.factionName);
        local description = QUEST_REPUTATION_REWARD_TOOLTIP:format(self.rewardAmount, self.factionName);
        local isAccountWide = API.IsAccountWideReputation(self.factionID);
        if addon.IS_LEGACY_ASCENSION then
            tooltip:SetText(title, HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
            if isAccountWide then
                tooltip:AddLine(REPUTATION_TOOLTIP_ACCOUNT_WIDE_LABEL, ACCOUNT_WIDE_FONT_COLOR.r, ACCOUNT_WIDE_FONT_COLOR.g, ACCOUNT_WIDE_FONT_COLOR.b);
            end
            tooltip:AddLine(description);
        else
            if GameTooltip_SetTitle then
                GameTooltip_SetTitle(tooltip, title, HIGHLIGHT_FONT_COLOR, wrapText);
            else
                tooltip:SetText(title, HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
            end
            if isAccountWide then

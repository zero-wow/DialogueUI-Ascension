...
local C_Container = addon.Legacy.C_Container or C_Container;
local C_Item = addon.Legacy.C_Item or C_Item;
local C_Map = addon.Legacy.C_Map or C_Map;
local C_QuestLog = addon.Legacy.C_QuestLog or C_QuestLog;
local C_Spell = addon.Legacy.C_Spell or C_Spell;
local Enum = addon.Legacy.Enum or Enum;
local EventRegistry = addon.Legacy.EventRegistry or EventRegistry;
local API = addon.API;
local L = addon.L;
...
do  -- Pixel
    ...
    local function UpdateTextureSliceScale(textureSlice)
        ...
        -- Texture regions do not expose SetScale on stock 3.3.5.  This
        -- adjustment is only relevant to Retail's texture-slice renderer, so
        -- leave the legacy texture at its layout-defined size.
        if textureSlice and textureSlice.SetScale then
...
do  -- NPC Interaction
    ...
    local TexturePrefix = "Interface/AddOns/DialogueUI-Ascension/Art/Icons/";
    ...
    local function GetInteractType(unit)
        if SetUnitCursorTexture and UnitExists(unit) then
    ...
    local function CreateModelScene()
        ...
        return ModelScene ~= nil
    end

    local function EvaluateUnitSize(unit)
        if not CreateModelScene() then
            return false
        end
    ...
    local function ParseLegacyGUID(guid)
        -- Original-Wrath GUIDs are 64-bit hexadecimal strings, for example
        -- 0xF1300030D4000A91.  The high GUID selects the object type and the
        -- following 24 bits contain the creature/game-object entry ID.
        local high, entry = guid and match(guid, "^0x(%x%x%x%x)(%x%x%x%x%x%x)");
        if high and entry then
            return LegacyGUIDTypes[string.upper(high)], tonumber(entry, 16)
        end
    end
    ...
    local function GetCreatureIDFromGUID(guid)
        ...
        if addon.IS_LEGACY_ASCENSION then
            local _, id = ParseLegacyGUID(guid);
            return id
        end
...
do  -- Quest
    ...
    local GetSuggestedGroupSize = GetSuggestedGroupSize or GetSuggestedGroupNum or AlwaysZero;
    ...
    local GetLogIndexForQuestID = C_QuestLog.GetLogIndexForQuestID or AlwaysNil;
    ...
    if addon.IS_LEGACY_ASCENSION and addon.Legacy.GetAvailableQuestInfo then
        API.GetAvailableQuestInfo = addon.Legacy.GetAvailableQuestInfo;
    ...
    if addon.IS_LEGACY_ASCENSION and addon.Legacy.GetActiveQuestID then
        API.GetActiveQuestID = addon.Legacy.GetActiveQuestID;
    elseif GetActiveQuestID then
    ...
    local function CompleteCurrentQuest(rewardChoiceID, isAutoComplete)
        ...
        if addon.IS_LEGACY_ASCENSION and addon.Legacy.MarkQuestRewardRequested then
    ...
    local function GetQuestIcon(questInfo)
        ...
        if not questInfo then
            return ICON_PATH.."IncompleteQuest.tga";
        end
        ...
        if questInfo.isOnQuest then
            if questInfo.isComplete then
                if questInfo.isCampaign then
                    file = "CompleteCampaignQuest.tga";
                elseif questInfo.isLegendary then
                    file = "CompleteLegendaryQuest.tga";
                elseif questInfo.isImportant then
                    file = "CompleteImportantQuest.tga";
                else
                    file = "CompleteQuest.tga";
                end
            else
                if questInfo.isCampaign then
                    file = "IncompleteCampaignQuest.tga";
                elseif questInfo.isLegendary then
                    file = "IncompleteLegendaryQuest.tga";
                elseif questInfo.isImportant then
                    file = "IncompleteImportantQuest.tga";
                elseif questInfo.isMeta then
                    file = "IncompleteMetaQuest.tga";
                else
                    file = "IncompleteQuest.tga";
                end
            end
            ...
            elseif questInfo.frequency == 2 then    --Enum.QuestFrequency.Weekly
                file = "WeeklyQuest.tga";
            elseif questInfo.frequency == 3 and not questInfo.isMeta then   ----Enum.QuestFrequency.ResetByScheduler
                file = "RepeatableScheduler.tga";    --TWW
            elseif  questInfo.repeatable then
                file = "RepeatableQuest.tga";
            else
                if questInfo.isCampaign then
                    file = "AvailableCampaignQuest.tga";
                elseif questInfo.isLegendary then
                    file = "AvailableLegendaryQuest.tga";
                elseif questInfo.isImportant then
                    file = "AvailableImportantQuest.tga";
                elseif questInfo.isMeta then
                    file = "AvailableMetaQuest.tga";
                else
                    file = "AvailableQuest.tga";
                end
            end
        end
    ...
    local DECOR_PATH = "Interface/AddOns/DialogueUI-Ascension/Art/ParchmentDecor/";
    local BackgroundDecors = {
        ["QuestBG-Dragonflight"] = "Dragonflight.tga",
        ["QuestBG-Azurespan"] = "Dragonflight.tga",
        ["QuestBG-EmeraldDream"] = "Dragonflight-Green.tga",
        ["QuestBG-Ohnplains"] = "Dragonflight-Green.tga",
        ["QuestBG-Thaldraszus"] = "Dragonflight-Bronze.tga",
        ["QuestBG-Walkingshore"] = "Dragonflight-Red.tga",
        ["QuestBG-ZaralekCavern"] = "Dragonflight.tga",
        ["QuestBG-ExilesReach"] = "Dragonflight.tga",

        ["QuestBG-Alliance"] = "Alliance.tga",
        ["QuestBG-Horde"] = "Horde.tga",

        ["QuestBG-Flame"] = "TWW-Flame.tga",
        ["QuestBG-Candle"] = "TWW-Candle.tga",
        ["QuestBG-Storm"] = "TWW-Storm.tga",
        ["QuestBG-Web"] = "TWW-Web.tga",
        ["QuestBG-1027"] = "TWW-Azeroth.tga",
        ["QuestBG-Rocket"] = "TWW-Rocket.tga",
        ["QuestBG-Fist"] = "TWW-Fist.tga",

        ["QuestBG-Sky"] = "MID-DarkHeart.tga",
    };
    ...
    local QUEST_TAG_NAME = {
        ...
        [Enum.QuestTag.Dungeon] = {L["Quest Type Dungeon"], "QuestTag-Dungeon.tga"},
        [Enum.QuestTag.Raid] = {L["Quest Type Raid"], "QuestTag-Raid.tga"},
        [Enum.QuestTag.Raid10] = {L["Quest Type Raid"], "QuestTag-Raid.tga"},
        [Enum.QuestTag.Raid25] = {L["Quest Type Raid"], "QuestTag-Raid.tga"},
...
do  -- Color
    ...
    local function GetItemQualityColor(quality)
        if QualityColors[quality] then
            ...
            local entry = ITEM_QUALITY_COLORS[quality] or ITEM_QUALITY_COLORS[1];
            if entry.color then
                return entry.color
            end
            return CreateColor(entry.r or 1, entry.g or 1, entry.b or 1, 1)
        end
    end
    ...
    local function GetColorizedAmountForCurrency(currencyID, rewardQuantity, useIcon)
        if WillCurrencyRewardOverflow(currencyID, rewardQuantity) then
            if useIcon then --For Small Button
                return "|TInterface/AddOns/DialogueUI-Ascension/Art/Icons/CurrencyOverflow.tga:0:0|t"..rewardQuantity.."|r"
            else
...
do  -- Model
    ...
    local function SetModelByUnit(model, unit)
        if addon.IS_LEGACY_ASCENSION then
            -- The 3.3.5 PlayerModel contract accepts only the unit token.
            model:SetUnit(unit);
            model.unit = unit;
            return
        end
    ...
    local function SetModelLight(model, enabled, omni, dirX, dirY, dirZ, ambIntensity, ambR, ambG, ambB, dirIntensity, dirR, dirG, dirB)
        if addon.IS_LEGACY_ASCENSION then
            return model:SetLight(
                enabled and 1 or nil,
                omni and 1 or 0,
                dirX or 0, dirY or 0, dirZ or 0,
                ambIntensity or 1, ambR or 1, ambG or 1, ambB or 1,
                dirIntensity or 1, dirR or 1, dirG or 1, dirB or 1
            )
        end
...
do  -- Chat Message
    ...
    local ADDON_ICON = "|TInterface\\AddOns\\DialogueUI-Ascension\\Art\\Icons\\Logo:0:0|t";
...
do  -- Tooltip
    ...
    if C_TooltipInfo then
        ...
        local function GetTooltipHyperlink()
            ...
            local spellName, spellRank, spellID = TP:GetSpell();
            if spellID then
        ...
        local function GetTooltipTexts()
            ...
            local itemLevel;
            local itemLink, tooltipID = GetTooltipHyperlink();
        ...
        local function AreItemsSameType(item1, item2)
            local _, itemType1, itemSubType1, _, _, classID1, subclassID1 = GetItemInfoInstant(item1);
            local _, itemType2, itemSubType2, _, _, classID2, subclassID2 = GetItemInfoInstant(item2);
            if classID1 ~= nil and classID2 ~= nil then
                return classID1 == classID2 and subclassID1 == subclassID2;
            end
            -- 3.3.5 GetItemInfo predates numeric class IDs but does return
            -- localized class/subclass names. Comparing those preserves the
            -- intended equipment-comparison grouping instead of treating two
            -- missing IDs as a match for every pair of items.
            return itemType1 ~= nil and itemType1 == itemType2 and itemSubType1 == itemSubType2;
        end
    ...
    local function GetItemEffect(itemLink)
        ...
        if (classID == 4 and subClassID == 0) or (classID == nil and itemEquipLoc and itemEquipLoc ~= "") then
...
do  -- Items
    ...
    local function GetItemClassification(item)
        ...
        if IsEquippableItem(item) or classID == 2 or classID == 4 then
            return "equipment"
...
do  -- System
    ...
    function API.ClearEditBoxFocus()
        ...
        if keyBoardFocus and keyBoardFocus.ClearFocus then
            addon.Legacy.SafeCall(keyBoardFocus.ClearFocus, keyBoardFocus);
        end
    end
